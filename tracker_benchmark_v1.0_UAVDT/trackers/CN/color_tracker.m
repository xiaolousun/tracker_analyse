function [finalPos, fps] = color_tracker(params, res_path, bSaveImage)
...
finalPos = zeros(numel(img_files), 4);
...
for frame = 1:num_frames
    ...
    if frame > 1
        ...
        if(isempty(row))
            [tempheight,tempwidth] = size(response);
            row = tempheight/2;
            col = tempwidth/2;
        end
    ...
    finalPos(frame, :) = round(rect_position);
    if visualization == 1
        if frame == 1,  %first frame, create GUI
%             figure('Number','off', 'Name','Tracker CN');

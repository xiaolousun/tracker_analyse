...
%         overLap(frame) = calcError(im, rect_position, p.bb_VOT(frame,:));
%         disp(['frame: ' num2str(frame) ', accuracy: ' num2str(mean(overLap(1:frame)))]);
%         figure(1),imshow(im);hold on;
%         text('Position',[10, 20],'String',['#' sprintf('%03d',frame)],'Color','r','FontSize',30); 
%         if(frame > 1)
%             text('Position',[rect_position(1), rect_position(2)-10],'String',num2str(roundn(max(response(:)),-1)),'Color','y','FontSize',10);         
%         end
%         rectangle('Position', rect_position, 'LineWidth', 3, 'edgecolor', 'r');
    end
    elapsed_time = toc;
